...
module.exports = function (str, msg, app) {
    ...
    if (commandLookup.success) {//Command lookup successful
        ...
        if (!command.admin || callerIsAdmin) {
            const content = command.fn(commandLookup.args, msg, app); //Run command fn
            ...
            return {
                content: `${app.strings.errorPermission} '${command.name}'`,
                type: "error"
            };
        }
        ...
        if (error.type === "missingCommand") {
            const content = [
                ...
                `${app.strings.infoSimilar} ${arrayToText(error.similar)}?`
            ].join("\n");
