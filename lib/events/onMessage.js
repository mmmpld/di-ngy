...
const sendableTypes = ["text", "code", "error"];
...
const sendText = function (app, msg, content) {
    msg.channel
        .sendCode(content)
        .then(message => app.log.info("SentText", message.content))
        .catch(err => app.log.warn("SentTextError", err));
};
...
const sendCode = function (app, msg, content) {
    msg.channel
        .sendCode("md", content)
        .then(message => app.log.info("SentCode", message.content))
        .catch(err => app.log.warn("SentCodeError", err));
};
